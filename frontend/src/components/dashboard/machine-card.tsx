...
export function MachineCard({
  ...
  const primary =
    machine.machine_type === "fan" && r?.speed_pct !== null && r?.speed_pct !== undefined
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate font-mono text-sm font-semibold">{machine.name}</p>
          <p className="truncate text-xs text-muted-foreground" title={machine.zone}>
            {machine.zone}
          </p>
        </div>
